...
void GetEulerAngles(float q0,float q1, float q2, float q3, float* yaw, float* pitch, float* roll)
{
	...
	else
	{
		...
		*yaw = atan2f(2*adbc, 1 - 2*(z2+x2));
		*pitch = asinf(2*abcd/unitLength);
		*roll = atan2f(2*acbd, 1 - 2*(y2+x2));
	}
}
...
void sensfusion6GetEulerRPY(float* roll, float* pitch, float* yaw)
{
  ...
  *yaw = atan2f(2*q1*q2 - 2*q0*q3, 2*q0*q0 + 2*q1*q1 - 1) * 180 / M_PI;
  *pitch = atanf(gx / __VSQRTF(gy*gy + gz*gz)) * 180 / M_PI;
  *roll = atanf(gy / __VSQRTF(gx*gx + gz*gz)) * 180 / M_PI;
}
*/
